        ...
        <div id="create-account">
            <form class="grid grid-cols-1 md:grid-cols-2 gap-4" action="{{ route('admin.accounts.store') }}" method="POST">
                ...
                <div class="mt-3">
                    ...
                    <select class="w-full px-3 py-2 border rounded bg-slate-50" name="role" required>
                    ...
                    @forelse($users ?? [] as $user)
                        <tr>
                            ...
                            <td class="py-2 px-4 border-b text-center">
                                ...
                                <a href="{{ route('admin.accounts.updatePassword', $user->id) }}"
                                    class="text-blue-600 hover:underline mx-1">Change Password</a>
                    ...
                    @forelse($removedUsers ?? [] as $user)
                        <tr>
                            ...
                            <td class="py-2 px-4 text-center">
                                <form method="POST" action="{{ route('admin.accounts.restore', $user->id) }}"
                                    class="inline">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="text-green-600 hover:underline mx-1">Restore</button>
                                </form>
                                <form method="POST" action="{{ route('admin.accounts.force-delete', $user->id) }}"
                                    class="inline">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:underline mx-1"
